...
class AdminController extends Controller
{
	...
	public function profiles(){
		...
		try {
			...
			DB::commit();
			return view('admin.profiles',compact('usuarios','IdEmpleado'));
		} catch (\Exception $e) {
			DB::rollback();
	...
	public function profiles_home(){
      ...
      // dd($usuario_creador);
			$IdEmpleado = Auth::guard('usuarios')->user()->idempleados;
			$usuarios = usuarios::select('rol','idusuario')->where('idusuario', $usuario_creador)->where('visible', 'SI')->where('idempleados', $IdEmpleado)->first();			

			date_default_timezone_set('America/Mexico_City');
			$fecha_acceso = date("Y-m-d H:i:s");
			$ip = $_SERVER['REMOTE_ADDR'];

			$InicioSesion = datos_inicio_sesion::createInicioSesion($fecha_acceso, $ip, '', $usuarios->idusuario, 'Usuario');
			...
			return back()->with('error', 'El perfil se encuentra en desarrollo '.$this->Direccion($usuarios->rol))->withInput();
		}
	}



	public static function Direccion($rol){
		$Direcciones = array(
			'logout' => 1,
      ...
      'CostoTotalVIN.index'      =>  'Costo_total_VIN',
      'Perfiles2/Chat_Panamotors/index.php'      =>  'Chat_Panamotors',
      'inventoryAdmin.index'      =>  'Inventario_Admin',// 'Perfiles2/Inventario_Admin/index.php'      =>  'Inventario_Admin',
      ...
      'CreditoCobranza.AdminController.index'      =>  'Credito_Cobranza',
      ...
      'Perfiles2/Wallet/index.php'      =>  'Wallet',
			'VPMovimientoExitoso.index'      =>  'Vista_Previa_Movimiento_Exitoso',
			'SalidasTallerTrackto.index'      =>  'Taller_Trucks',
  );
