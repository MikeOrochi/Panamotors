	...
	<div class="col-sm-9 header-menu pt-2 pb-0 pr-0 pl-0">
		...
		<div class="flex-header-menu justify-content-end col-12 col-sm-8 col-md-8 col-lg-10 pt-1">
			<div class="" style="width:25%;">
				<select class="js-example-basic-single" name="buscador" id="buscador" style="width: 100%;">
					<option value="contactos" selected>Contactos</option>
					<option value="carros">VIN</option>
				</select>
			</div>
			<div class=""  style="width:75%;">
				<select class="js-example-basic-single form-control buscar1 mSearch" id="search_contacts_cars" style="width: 100%;"></select>
		...
		<div class="inner-sidebar mr-3">
			...
			<div class="sidebar-menu-container">
				<ul class="sidebar-menu mb-4">
					...
					<li class="parent">
						<a href="{{route('admin.index')}}" class=""><i class="fas fa-home mr-3"></i>
							<span class="none">Dashboard </span>
						</a>
					</li>
					<li class="parent">
						<a href="{{route('pagare.index')}}" class=""><i class="fas fa-money-check mr-3"></i>
							<span class="none">Pagares </span>
						</a>
					</li>
					<li class="parent">
						<a href="#" onclick="toggle_menu('dashboard'); return false" class=""><i class="fa fa-users" aria-hidden="true"></i>
							<span class="none">Proveedores <i class="fa fa-angle-down pull-right align-bottom"></i></span>
						</a>
						<ul class="children" id="dashboard">

							<li class="child"><a href="{{route('provider.index')}}" class="ml-4"><i class="fa fa-users" aria-hidden="true"></i> Agregar proveedor</a></li>

							<li class="child"><a href="{{route('wallet.index')}}" class="ml-4"><i class="fa fa-users" aria-hidden="true"></i> Cartera</a></li>

						</ul>
					</li>
					{{-- <li class="parent">
					<a href="#" onclick="toggle_menu('dashboard'); return false" class=""><i class="fas fa-check-square mr-3"> </i>
					<span class="none">Orden de Compra <i class="fa fa-angle-down pull-right align-bottom"></i></span>
				</a>
				<ul class="children" id="dashboard">

				<li class="child"><a href="agregar_orden_compra.php" class="ml-4"><i class="fas fa-check-square mr-2"></i> Agregar Orden Compra</a></li>

				<li class="child"><a href="detalle_orden_compras.php" class="ml-4"><i class="fas fa-check-square mr-2"></i> Detalle Orden de Compra</a></li>
...
$( document ).ready(function() {
	...
	fetch("{{route('pagare.notification')}}", {
		headers: {
	...
	.then(function(response){
		...
		var Ruta = '{{route('pagare.index')}}';
		var RutaSemana = '{{route('pagare.week')}}';

		if (response != null ) {
			var NumNotificaciones = 0;

			if (response.Proximos > 0) {
				NumNotificaciones++;
				$('#MisNotificaciones').append(`<a class="dropdown-item" href="`+Ruta+`">Tienes <strong>`+response.Proximos+`</strong> pagares <b>por vencer de hoy a 7 días</b></a>`)
			}
			if (response.Vencidos > 0) {
				NumNotificaciones++;
				$('#MisNotificaciones').append(`<a class="dropdown-item" href="`+Ruta+`">Tienes <strong>`+response.Vencidos+`</strong> pagares <b>vencidos</b></a>`)
			}

			if (NumNotificaciones > 0) {
				document.getElementById('Notificaciones').style.background  = '#ff7575';
				document.getElementById('Notificaciones').textContent = NumNotificaciones;
			}
		}
	});
